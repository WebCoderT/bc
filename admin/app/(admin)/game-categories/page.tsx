...
import {
  createAdminGameCategory,
  deleteAdminGameCategory,
  fetchAdminGameCategories,
  isAdminAuthError,
  updateAdminGameCategory,
  type AdminGameCategory,
  type SaveAdminGameCategoryInput,
  type UpdateGameCategoryInput,
} from "@/app/lib/admin-api";
import type { CategoryItem } from "@/app/types/ui";

function mapAdminCategoryToCategoryItem(
...
export default function GameCategoriesRoute() {
  ...
  const handlePersistCatalog = useCallback((nextCatalog: typeof catalog) => {
    const normalizedCatalog = writeGameCatalog(nextCatalog);
    setCatalog(normalizedCatalog);
  }, []);

  const syncCategoriesToCatalog = useCallback(
    ...
    [catalog.games, handlePersistCatalog],
  );

  const loadCategories = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await fetchAdminGameCategories(session.accessToken);
      syncCategoriesToCatalog(
        response.items.map(mapAdminCategoryToCategoryItem),
      );
      setLoadError("");
    } catch (error) {
      ...
      setLoadError(error instanceof Error ? error.message : "读取游戏分类失败");
    } finally {
      setIsLoading(false);
    }
  }, [logout, session.accessToken, syncCategoriesToCatalog]);
  ...
  const handleSaveCategory = async (input: CategoryFormInput) => {
    ...
    try {
      ...
      if (editingCategory) {
        await updateAdminGameCategory(session.accessToken, editingCategory.id, {
          ...payload,
          status:
            payload.status as unknown as UpdateGameCategoryInput["status"],
        });
      } else {
        await createAdminGameCategory(session.accessToken, {
          ...payload,
          status:
            payload.status as unknown as SaveAdminGameCategoryInput["status"],
        });
      }
  ...
  const handleDeleteCategory = (categoryId: number) => {
    ...
    void (async () => {
      try {
        await deleteAdminGameCategory(session.accessToken, categoryId);
        await loadCategories();
      } catch (error) {
        if (isAdminAuthError(error)) {
          logout();
        ...
        setLoadError(error instanceof Error ? error.message : "删除分类失败");
      }
    })();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {!isLoading && categoryMetrics.length === 0 ? (
